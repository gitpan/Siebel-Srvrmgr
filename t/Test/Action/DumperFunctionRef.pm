...
sub class_methods : Test(+3) {
    ...
    is( ref($y), 'CODE', '$y is a code reference' );

    #    stdout_like( $y->($test), $regex,  'got correct dumped content');

    stdout_like( sub { $test->{action}->do( \@data ); },
        $regex, 'got correct dumped content' );
